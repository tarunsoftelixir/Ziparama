...
public class EditPin extends FragmentActivity implements
	...
	public Loader<List<LatLongData>> onCreateLoader(int id, Bundle bundle) {
		...
		progress.show();
		if (intent != null && intent.getStringExtra("GET") != null
				&& intent.getStringExtra("GET").equals("ALL_PINS"))
	...
	public void onLoadFinished(Loader<List<LatLongData>> loader,
		...
		if (list != null) {
			for (int i = 0; i < list.size(); i++) {
				...
				map.put("ImagePath", list.get(i).getImageUrl() + "");
				map.put("Category", list.get(i).getCategory());
				map.put("SubCategory", list.get(i).getSubcategory());
				Log.d("list_data", map + "");
	...
	public void attachAdapetr() {
		if (editpinarray.size() > 0) {
			Toast.makeText(EditPin.this, "Arrayisnotnull", Toast.LENGTH_LONG)
					.show();
			adapteredit = new MyAdapterEdit(EditPin.this, editpinarray);
			list.setAdapter(adapteredit);
		} else {
			Toast.makeText(EditPin.this, "nullarraylist", Toast.LENGTH_LONG)
					.show();
